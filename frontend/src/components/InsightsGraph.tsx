...
import type { InsightsConfig, InsightsDataPoint, HeatMapDataPoint, AxisType, PresetConfig } from '../types/metrics';
...
const InsightsGraph: React.FC<InsightsGraphProps> = ({
  ...
  const [selectedPoint, setSelectedPoint] = useState<InsightsDataPoint | null>(null);
  ...
  const handleViewModeChange = (_: React.MouseEvent<HTMLElement>, mode: 'scatter' | 'heatmap' | null) => {
    if (mode) {
  ...
  const renderTooltip = (value: any, name: string) => {
    const label = getAxisLabel(config.xAxis === name ? config.xAxis : config.yAxis);
  ...
  return (
    <Box sx={{ mt: 4, pt: 2, borderTop: 1, borderColor: 'divider' }}>
            ...
            {config.viewMode === 'scatter' ? (
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <XAxis
                  ...
                  label={{ value: getAxisLabel(config.xAxis), position: 'insideBottom', offset: -10 }}
                ...
                <YAxis
                  ...
                  label={{ value: getAxisLabel(config.yAxis), angle: -90, position: 'insideLeft' }}
                  tickFormatter={(value, index) =>
                    formatValue(value)
                  }
                />
              ...
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" dataKey="x" label={{ value: getAxisLabel(config.xAxis), position: 'insideBottom', offset: -10 }} />
                <YAxis type="number" dataKey="y" label={{ value: getAxisLabel(config.yAxis), angle: -90, position: 'insideLeft' }} />
                <Tooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
      ...
      <Dialog open={!!selectedPoint} onClose={handleCloseModal} maxWidth="sm" fullWidth>
          ...
          {logDetails ? (
            <DialogContentText>
              <Box sx={{ mt: 2 }}>
                ...
                <p><strong>Total Tokens:</strong> {logDetails.total_tokens}</p>
                <p><strong>Duration:</strong> {logDetails.duration_ms} ms</p>
                <p><strong>Cache Creation:</strong> {logDetails.cache_creation_input_tokens || 0}</p>
                <p><strong>Cache Read:</strong> {logDetails.cache_read_input_tokens || 0}</p>
              </Box>
            </DialogContentText>
          ) : (
            <CircularProgress />
